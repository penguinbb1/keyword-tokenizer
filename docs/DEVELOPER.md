# 开发者文档 - 多语言电商关键词分词与标注系统

## 📋 目录

1. [项目概述](#项目概述)
2. [系统架构](#系统架构)
3. [目录结构](#目录结构)
4. [核心处理流程](#核心处理流程)
5. [模块详解](#模块详解)
6. [词典系统](#词典系统)
7. [扩展指南](#扩展指南)
8. [性能优化](#性能优化)

---

## 项目概述

### 解决的问题

电商关键词（如商品标题、搜索词）通常具有以下特点：
- **多语言混合**：`Nike ランニングシューズ 26.5cm` 同时包含英语、日语、数字
- **无明确分隔**：`thermoleggingsdamen` 德语复合词没有空格
- **需要语义标注**：需要识别出 `Nike=品牌词`、`ランニングシューズ=商品词`

### 核心功能

1. **智能分词**：根据字符类型自动选择分词器（中文/日语/欧洲语言）
2. **语义标注**：将每个词标注为 品牌词/商品词/人群词/场景词/颜色词/尺寸词/卖点词/属性词
3. **置信度评估**：给出每个标注的可信程度
4. **AI 增强**：对低置信度词调用 Claude API 补充标注

### 技术栈

- **Python 3.9+**
- **jieba**：中文分词
- **sudachipy**：日语分词
- **FastAPI**：API 服务
- **Claude API**：AI 增强标注

---

## 系统架构

```
┌─────────────────────────────────────────────────────────────────┐
│                         输入关键词                                │
│              "Nike ランニングシューズ メンズ 26.5cm"                │
└─────────────────────────────────────────────────────────────────┘
                                │
                                ▼
┌─────────────────────────────────────────────────────────────────┐
│                      1. 预处理 (Preprocessor)                    │
│                   - 统一编码、清理特殊字符                          │
└─────────────────────────────────────────────────────────────────┘
                                │
                                ▼
┌─────────────────────────────────────────────────────────────────┐
│                  2. 固定短语提取 (SpanExtractor)                  │
│              - 提取品牌词、已知短语，标记为"已锁定"                   │
│              - "Nike" → [品牌词, 0.95]                           │
└─────────────────────────────────────────────────────────────────┘
                                │
                                ▼
┌─────────────────────────────────────────────────────────────────┐
│                  3. 脚本分段 (ScriptSegmenter)                    │
│              - 按字符类型切分：Latin / Kana / CJK / Number         │
│              - "ランニングシューズ メンズ" → [KANA段]                │
│              - "26.5cm" → [Latin+Number段]                       │
└─────────────────────────────────────────────────────────────────┘
                                │
                                ▼
┌─────────────────────────────────────────────────────────────────┐
│                    4. 分段分词 (Tokenizers)                       │
│              - KANA段 → JapaneseTokenizer (Sudachi)              │
│              - CJK段 → ChineseTokenizer (jieba)                  │
│              - Latin段 → EuropeanTokenizer (空格分词)             │
└─────────────────────────────────────────────────────────────────┘
                                │
                                ▼
┌─────────────────────────────────────────────────────────────────┐
│                 5. 日语复合词合并 (JapaneseCompoundMerger)         │
│              - "ランニング" + "シューズ" → "ランニングシューズ"       │
│              - 只合并词典中存在的复合词                             │
└─────────────────────────────────────────────────────────────────┘
                                │
                                ▼
┌─────────────────────────────────────────────────────────────────┐
│                  6. 短语合并 (PhraseMerger)                       │
│              - "long" + "sleeve" → "long sleeve"                 │
│              - 欧洲语言固定搭配合并                                 │
└─────────────────────────────────────────────────────────────────┘
                                │
                                ▼
┌─────────────────────────────────────────────────────────────────┐
│                  7. 标签标注 (EnhancedTagger)                     │
│              - 多层策略：词典 → 规则 → 启发式 → 默认                 │
│              - 西班牙语词形归一化                                   │
│              - 上下文调整                                         │
└─────────────────────────────────────────────────────────────────┘
                                │
                                ▼
┌─────────────────────────────────────────────────────────────────┐
│                  8. AI 增强 (可选) (AIEnhancer)                   │
│              - 对置信度 ≤ 0.6 的词调用 Claude API                  │
│              - 结果进入候选池，人工确认后写入词典                     │
└─────────────────────────────────────────────────────────────────┘
                                │
                                ▼
┌─────────────────────────────────────────────────────────────────┐
│                           输出结果                                │
│  [                                                               │
│    {"token": "Nike", "tag": "品牌词", "confidence": 0.95},        │
│    {"token": "ランニングシューズ", "tag": "商品词", "confidence": 0.9},│
│    {"token": "メンズ", "tag": "人群词", "confidence": 0.9},         │
│    {"token": "26.5cm", "tag": "尺寸词", "confidence": 0.95}        │
│  ]                                                               │
└─────────────────────────────────────────────────────────────────┘
```

---

## 目录结构

```
keyword-tokenizer/
│
├── api/                          # API 服务层
│   ├── __init__.py
│   ├── main.py                   # FastAPI 应用入口
│   ├── models/                   # 请求/响应模型定义
│   │   ├── request.py
│   │   └── response.py
│   └── routes/                   # API 路由
│       ├── tokenize.py           # 分词接口
│       └── dictionary.py         # 词典管理接口
│
├── config/                       # 配置管理
│   ├── __init__.py
│   └── settings.py               # 配置类（读取 .env）
│
├── core/                         # 核心处理模块 ⭐
│   ├── __init__.py
│   ├── enhanced_pipeline.py      # 主处理流水线（入口）
│   ├── enhanced_tagger.py        # 增强版标签标注器
│   ├── preprocessor.py           # 文本预处理
│   ├── script_segmenter.py       # 脚本分段器
│   ├── span_extractor.py         # 固定短语提取器
│   ├── phrase_merger.py          # 欧洲语言短语合并
│   ├── japanese_compound_merger.py  # 日语复合词合并
│   ├── spanish_normalizer.py     # 西班牙语词形归一化
│   ├── language_detector.py      # 语言检测
│   ├── tagger.py                 # 基础标签标注器
│   ├── pipeline.py               # 基础流水线（旧版）
│   └── tokenizers/               # 分词器
│       ├── __init__.py
│       ├── base.py               # 基类
│       ├── chinese.py            # 中文分词（jieba）
│       ├── japanese.py           # 日语分词（sudachi）
│       └── european.py           # 欧洲语言分词
│
├── services/                     # 服务层
│   ├── __init__.py
│   ├── dictionary_manager.py     # 词典管理器
│   ├── ai_enhancer.py            # AI 增强服务（基础版）
│   ├── ai_enhancer_v2.py         # AI 增强服务（批量版）
│   └── candidate_pool.py         # AI 结果候选池
│
├── dictionaries/                 # 词典数据 📚
│   ├── products.json             # 商品词典
│   ├── brands.json               # 品牌词典
│   ├── brands/                   # 品牌子词典
│   │   ├── global.json
│   │   ├── zh.json
│   │   └── ja.json
│   ├── audiences.json            # 人群词典
│   ├── scenarios.json            # 场景词典
│   ├── colors.json               # 颜色词典
│   ├── features.json             # 卖点词典
│   ├── attributes.json           # 属性词典
│   └── ja.json                   # 日语专用词典
│
├── dict_expansion/               # 词典扩充模块 📚
│   ├── apply_expansion.py        # 扩充脚本（读取 JSON 导入词典）
│   ├── english.json              # 英语扩充词典
│   ├── french.json               # 法语扩充词典
│   ├── german.json               # 德语扩充词典
│   ├── japanese.json             # 日语扩充词典
│   └── spanish.json              # 西班牙语扩充词典
│
├── scripts/                      # 工具脚本
│   ├── batch_test.py             # 批量测试（基础版）
│   ├── batch_test_v2.py          # 批量测试（AI 优化版）
│   ├── safe_dict_expand.py       # 安全词典扩充（日语复合词等）
│   ├── import_ai_tags.py         # 导入 AI 标注结果到词典
│   └── security_check.py         # GitHub 开源安全检查
│
├── tests/                        # 测试
│   ├── __init__.py
│   ├── test_core.py
│   └── test_tokenizer.py
│
├── docs/                         # 文档
│   ├── DEVELOPER.md              # 开发者文档（本文件）
│   └── V2_ARCHITECTURE.md        # 架构设计文档
│
├── .env.example                  # 环境变量示例
├── .gitignore                    # Git 忽略配置
├── requirements.txt              # Python 依赖
├── run.py                        # 启动脚本
└── README.md                     # 项目说明
```

---

## 核心处理流程

### 入口文件：`core/enhanced_pipeline.py`

```python
class EnhancedPipeline:
    async def process(self, keyword: str, language: str = None) -> Dict:
        # 1. 预处理
        cleaned, _ = self.preprocessor.process(keyword)
        
        # 2. 固定短语提取（品牌词等）
        spans, locked_ranges = self.span_extractor.extract(cleaned)
        
        # 3. 脚本分段
        segments = self.segmenter.segment(cleaned)
        
        # 4. 对每段进行分词
        for segment in segments:
            tokenizer = self.get_tokenizer(segment.script)
            tokens = tokenizer.tokenize(segment.text)
        
        # 5. 短语合并（欧洲语言）
        merged_tokens = self.phrase_merger.merge(tokens)
        
        # 6. 标签标注
        results = self.tagger.tag(tokens, language=language)
        
        # 7. AI 增强（可选）
        if self.ai_enhancer:
            results = await self._apply_ai_enhancement(results)
        
        return self._format_output(results)
```

---

## 模块详解

### 1. 预处理器 (`core/preprocessor.py`)

**功能**：清理输入文本

```python
class Preprocessor:
    def process(self, text: str) -> Tuple[str, Dict]:
        # - 统一 Unicode 编码（NFKC）
        # - 全角转半角
        # - 清理多余空格
        # - 记录处理过程
        return cleaned_text, record
```

### 2. 脚本分段器 (`core/script_segmenter.py`)

**功能**：按字符类型将混合语言文本切分

```python
class ScriptSegmenter:
    def segment(self, text: str) -> List[Segment]:
        # 输入: "Nike ランニング 26.5cm"
        # 输出: [
        #   Segment("Nike", LATIN),
        #   Segment("ランニング", KANA),
        #   Segment("26.5cm", LATIN)
        # ]
```

**支持的脚本类型**：
- `CJK`：中文汉字
- `KANA`：日语假名
- `HANGUL`：韩语
- `LATIN`：拉丁字母
- `NUMBER`：数字

### 3. 分词器 (`core/tokenizers/`)

#### 中文分词器 (`chinese.py`)
```python
class ChineseTokenizer:
    def tokenize(self, text: str) -> List[Token]:
        # 使用 jieba 分词
        # 支持动态添加词（品牌词等）
```

#### 日语分词器 (`japanese.py`)
```python
class JapaneseTokenizer:
    def tokenize(self, text: str) -> List[Token]:
        # 使用 Sudachi (Mode C) 分词
        # Mode C 保留较长的复合词
```

#### 欧洲语言分词器 (`european.py`)
```python
class EuropeanTokenizer:
    def tokenize(self, text: str) -> List[Token]:
        # 基于空格和标点分词
        # 处理德语复合词（-分隔）
```

### 4. 日语复合词合并器 (`core/japanese_compound_merger.py`)

**功能**：解决日语过度分词问题

```python
class JapaneseCompoundMerger:
    def merge(self, tokens: List[str]) -> List[str]:
        # 输入: ["ランニング", "シューズ"]
        # 输出: ["ランニングシューズ"]  # 仅当词典中存在时
        
        # 策略：
        # 1. 预定义复合词词典匹配
        # 2. 词尾规则合并（巻+き → 巻き）
        # 3. 片假名商品词合并（需词典验证）
```

**关键设计**：只合并词典中存在的词，避免生成无效复合词

### 5. 西班牙语归一化器 (`core/spanish_normalizer.py`)

**功能**：处理西班牙语复数和性别变化

```python
class SpanishNormalizer:
    def normalize(self, word: str) -> NormalizedWord:
        # 输入: "inalámbricas"
        # 输出: "inalámbrico"
        
        # 规则：
        # 1. 复数还原：-s, -es → 单数
        # 2. 性别归一：-a → -o（验证后）
```

### 6. 短语合并器 (`core/phrase_merger.py`)

**功能**：合并欧洲语言固定搭配（263 条预定义短语）

```python
class PhraseMerger:
    def merge(self, tokens: List[str]) -> List[MergedToken]:
        # 输入: ["large", "waterproof", "duffle", "bag"]
        # 输出: ["large", "waterproof", "duffle bag"]
        
        # 预定义搭配类型：
        # - 商品词短语 (120条): card stock, duffle bag, yoga pants
        # - 属性词短语 (73条): high waist, long sleeve, memory foam
        # - 卖点词短语 (38条): noise cancelling, quick dry
        # - 人群词短语 (14条): para mujer, pour femme
        # - 尺寸词短语 (9条): queen size, king size
        # - 场景词短语 (9条): work out, outdoor sports
```

**多语言支持**：
- 英语: high waist, long sleeve, duffle bag
- 德语: mit taschen, mit kapuze, hohe taille
- 法语: taille haute, manches longues, pour femme
- 西班牙语: manga larga, cintura alta, para mujer

### 7. 增强版标签标注器 (`core/enhanced_tagger.py`)

**功能**：多层策略标注 + 置信度评估

```python
class EnhancedTagger:
    def tag(self, tokens: List[str], language: str = None) -> List[TagResult]:
        # 标注策略（优先级从高到低）：
        # 1. 词典匹配（confidence: 0.9-0.95）
        # 2. 正则模式（颜色词、尺寸词）
        # 3. 规则推断（关键字列表）
        # 4. 启发式推断（词缀、位置）
        # 5. 默认标签（confidence: 0.5）
        
        # 特殊处理：
        # - 日语复合词合并
        # - 西班牙语归一化匹配
        # - 上下文调整
```

**标签类型**：
| 标签 | 说明 | 示例 |
|------|------|------|
| 品牌词 | 品牌名称 | Nike, Sony, ナイキ |
| 商品词 | 商品品类 | leggings, シューズ, 背包 |
| 人群词 | 目标人群 | damen, メンズ, kids |
| 场景词 | 使用场景 | running, アウトドア |
| 颜色词 | 颜色 | schwarz, 黒, blue |
| 尺寸词 | 尺寸规格 | 26.5cm, XL, 32GB |
| 卖点词 | 产品特性 | waterproof, 軽量 |
| 属性词 | 其他属性 | with, für, long |

### 8. AI 增强服务 (`services/ai_enhancer_v2.py`)

**功能**：对低置信度词调用 Claude API 补充标注

```python
class AIEnhancer:
    async def process_batch(self, words: List[str], context: str) -> Dict:
        # 构造 Prompt，调用 Claude API
        # 返回每个词的标签和置信度
        
        # 输入: ["pilou", "gefüttert"]
        # 输出: {
        #   "pilou": {"tag": "属性词", "confidence": 0.85},
        #   "gefüttert": {"tag": "卖点词", "confidence": 0.9}
        # }
```

---

## 词典扩充模块 (`dict_expansion/`)

### 设计理念

将词典扩充与主代码分离，便于：
- 按语言独立维护
- 版本控制友好（JSON 文件易于 diff）
- 支持批量审核后再导入

### 扩充文件格式

```json
{
  "language": "german",
  "description": "德语词典扩充 - 基于批量测试低置信度词汇分析",
  "stopwords": ["mit", "für", "und", "der", "die"],
  "entries": [
    {"word": "radlerhose", "tag": "商品词", "confidence": 0.95, "note": "骑行裤"},
    {"word": "baumwolle", "tag": "属性词", "confidence": 0.9, "note": "棉"}
  ]
}
```

**字段说明**：
- `language`：语言标识
- `stopwords`：虚词列表（导入时自动跳过）
- `entries`：词条列表
  - `word`：词汇
  - `tag`：标签类型
  - `confidence`：置信度
  - `note`：中文注释（便于理解）

### 使用方法

```bash
# 预览（不实际修改）
python3 dict_expansion/apply_expansion.py --dry-run

# 实际导入
python3 dict_expansion/apply_expansion.py
```

### 工作流程

1. 运行批量测试，发现低置信度词
2. 分析后手动添加到对应语言的 JSON 文件
3. 运行 `apply_expansion.py` 导入词典
4. 重新测试验证效果

---

## 词典系统

### 词典格式

```json
{
  "name": "商品词典",
  "description": "商品品类名称（多语言）",
  "entries": [
    {"word": "leggings", "confidence": 0.95},
    {"word": "シューズ", "confidence": 0.95},
    {"word": "背包", "confidence": 1.0}
  ]
}
```

### 词典管理器 (`services/dictionary_manager.py`)

```python
class DictionaryManager:
    def load_all(self):
        # 加载所有词典文件
        # 构建词索引（快速查找）
    
    def contains(self, dict_name: str, word: str) -> bool:
        # 检查词是否在词典中
    
    def add_word(self, dict_name: str, word: str, confidence: float):
        # 添加新词到词典
```

---

## 扩展指南

### 添加新语言支持

1. 在 `core/tokenizers/` 创建新分词器
2. 在 `script_segmenter.py` 添加字符类型识别
3. 在 `enhanced_tagger.py` 添加语言特定规则
4. 在 `dictionaries/` 添加语言词典

### 添加新标签类型

1. 在 `enhanced_tagger.py` 的 `TagType` 枚举添加
2. 在 `dictionaries/` 创建对应词典
3. 在 `_match_dictionary()` 添加映射

### 添加新词典

```bash
# 1. 创建词典文件
echo '{
  "name": "新词典",
  "description": "说明",
  "entries": []
}' > dictionaries/new_dict.json

# 2. 在 dictionary_manager.py 注册
```

---

## 性能优化

### 当前优化

1. **词典索引**：使用 dict 哈希表快速查找
2. **延迟加载**：分词器按需初始化
3. **批量处理**：AI 增强支持批量调用

### 优化建议

1. **缓存**：对高频词添加 LRU 缓存
2. **并行**：使用 asyncio 并行处理多个关键词
3. **预编译**：正则表达式预编译

---

## 调试技巧

### 查看分词过程

```python
from core.script_segmenter import ScriptSegmenter

segmenter = ScriptSegmenter()
segments = segmenter.segment("Nike ランニング 26.5cm")
for seg in segments:
    print(f"[{seg.script.value}] {seg.text}")
```

### 查看标注详情

```python
# 在 EnhancedTagger.tag() 中查看 all_candidates
for result in results:
    print(f"{result.token}: {result.primary_tag} ({result.confidence})")
    for cand in result.all_candidates:
        print(f"  - {cand.tag}: {cand.confidence} ({cand.method})")
```

---

## 常见问题

### Q: 为什么某个词标注错误？

1. 检查词典是否包含该词
2. 检查是否有规则冲突
3. 查看 `all_candidates` 了解标注过程

### Q: 如何提高标注准确率？

1. 扩充词典（最直接）
2. 添加语言特定规则
3. 使用 AI 增强

### Q: 如何处理新的复合词？

1. 日语：添加到 `products.json`，合并器会自动识别
2. 欧洲语言：添加到 `phrase_merger.py` 的预定义搭配

---

*文档版本: 1.1 | 最后更新: 2025-01*

**更新日志**：
- v1.1: 添加短语合并模块（263 条短语）、Google Taxonomy 导入工具
- v1.0: 初始版本

---

## 工具脚本详解 (`scripts/`)

### 核心脚本

| 脚本 | 用途 | 使用频率 |
|------|------|----------|
| `batch_test_v2.py` | 批量测试关键词，生成结果报告 |
| `import_ai_tags.py` | 将 AI 标注结果导入词典 |
| `import_google_taxonomy.py` | 导入 Google 商品分类词典 |
| `safe_dict_expand.py` | 安全扩充词典（日语复合词等） |
| `security_check.py` | GitHub 开源前安全检查 |

### 辅助脚本

| 脚本 | 用途 | 说明 |
|------|------|------|
| `batch_test.py` | 旧版批量测试 | 已被 v2 替代 |
| `test_v2.py` | 单条测试调试 | 开发调试用 |
| `test_api_model.py` | API 模型测试 | 开发调试用 |
| `expand_dictionary.py` | 词典扩展 | 功能与 dict_expansion 重叠 |
| `expand_from_results.py` | 从结果扩展词典 | 功能与 import_ai_tags 重叠 |

### 使用示例

```bash
# 1. 批量测试（最常用）
python scripts/batch_test_v2.py keywords.csv -o results.json --no-ai

# 2. 导入 AI 标注结果
python scripts/import_ai_tags.py results_ai.json --dry-run  # 预览
python scripts/import_ai_tags.py results_ai.json            # 实际导入

# 3. 导入 Google Product Taxonomy
python scripts/import_google_taxonomy.py --lang en --dry-run  # 预览英语
python scripts/import_google_taxonomy.py --lang en            # 导入英语
python scripts/import_google_taxonomy.py --lang de            # 导入德语
python scripts/import_google_taxonomy.py --lang fr            # 导入法语

# 4. 安全扩充词典
python scripts/safe_dict_expand.py --apply

# 5. 开源前检查
python scripts/security_check.py
```

### Google Taxonomy 导入说明

Google Product Taxonomy 是 Google 官方的商品分类体系，包含 5,000+ 商品类目，支持 20+ 语言。

**下载完整版 Taxonomy 文件**：
```bash
# 英语
curl -o scripts/taxonomy_data/taxonomy.en-US.txt \
  "https://www.google.com/basepages/producttype/taxonomy.en-US.txt"

# 德语
curl -o scripts/taxonomy_data/taxonomy.de-DE.txt \
  "https://www.google.com/basepages/producttype/taxonomy.de-DE.txt"

# 法语、西班牙语、日语等同理
```

**支持的语言代码**：
- `--lang en` (en-US)
- `--lang de` (de-DE)
- `--lang fr` (fr-FR)
- `--lang es` (es-ES)
- `--lang ja` (ja-JP)
- `--lang zh` (zh-CN)
